...
namespace UnitTestDeadCodeRemover
{
    ...
    public class UnitTest1
    {
        ...
        public static void TestInitialize(TestContext context0)
        {
            ...
            try
            {
                ...
                var solution = workspace
                    .OpenSolutionAsync(
                        @"C:\Users\wnms2\source\repos\DeadCodeRemover\ClassLibrary4Roslyn.sln",
                        new ConsoleProgressReporter()
                    )
                    .Result;
                var projects = solution.Projects.Where(p =>
                    p.FilePath
                    == @"C:\Users\wnms2\source\repos\DeadCodeRemover\ClassLibrary4Roslyn\ClassLibrary4Roslyn.csproj"
                );
                if (projects.Count() == 0)
                {
                    throw new ArgumentException(
                        $"Cannot find project ClassLibrary4Roslyn.csproj in solution,"
                    );
                }
        ...
        {
            var type0 = types.FirstOrDefault(t =>
                t.FullName == "ClassLibrary4Roslyn.EnumHasReferences"
            );
            Assert.AreEqual(3, type0?.TypesUsingMe.Count());
            var type1 = types.FirstOrDefault(t =>
                t.FullName == "ClassLibrary4Roslyn.EnumHasNoReferences"
            );
        ...
        {
            var type0 = types.FirstOrDefault(t =>
                t.FullName == "ClassLibrary4Roslyn.StaticClassWithExtensionMethod"
            );
            Assert.AreEqual(false, type0?.IsDead);
        }
        ...
        {
            var type0 = types.FirstOrDefault(t =>
                t.FullName == "ClassLibrary4Roslyn.ClassWithOnlyConstants"
            );
        ...
        {
            var type0 = types.FirstOrDefault(t =>
                t.FullName == "ClassLibrary4Roslyn.ClassUsedAsAttribute"
            );
            Assert.AreEqual(1, type0?.TypesUsingMe.Count());
        }
        ...
        {
            var type0 = types.FirstOrDefault(t =>
                t.FullName == "ClassLibrary4Roslyn.ClassWithSelfReferences"
            );
            Assert.AreEqual(0, type0?.TypesUsingMe.Count());
        }
        ...
        {
            var type0 = types.FirstOrDefault(t =>
                t.FullName == "ClassLibrary4Roslyn.MyUserControl"
            );
            Assert.AreEqual(0, type0?.TypesUsingMe.Count());
        }
